...
class ESN:
    ...
    def reservoir_state(self,u,leaky_rate=1,input_gain=0.1,spectral_radius=1,feedback_gain=0,d=[-1],bias=True):
        ...
        for i in range(u.shape[1]):
            if i == 0:
                ...
                state[:,i] += leaky_rate*np.tanh(input_gain*self.w_in.T@u[:,i] + spectral_radius*self.w_res.T@state[:,i-1] + feedback_gain*self.w_back.T@d[:,i-1])
    ...
    def weights_visualization(self,input_weights=True,reservoir_weights=True,back_weights=False,out_weights=False,display_in='rows'):
        """
            ...
            for j in input_gain_range:
                for k in spectral_radius_range:
                    for l in feedback_gain_range:
                        count += 1
                        state = self.reservoir_state(u,leaky_rate=i,input_gain=j,spectral_radius=k,feedback_gain=l,d=_d)
                        self.fit(state,y)
                        ...
                        print('{}/{}  -->  NRMSE: {} MSE: {}  -->  params:({},{},{},{})'.format(count,leaky_range.size*input_gain_range.size*spectral_radius_range.size*feedback_gain_range.size,round(nrmsError,4),round(msError,4),i,j,k,l))
                        print('best  NRMSE: {} MSE: {}  --  params:({},{},{},{})'.format(round(lowest_nrmse,4),round(lowest_mse,4),best_parameters_nrmse[0][1],best_parameters_nrmse[0][2],best_parameters_nrmse[0][3],best_parameters_nrmse[0][4]))
